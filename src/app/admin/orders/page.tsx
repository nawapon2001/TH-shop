...
import {
  ...
  CheckCircle2, Clock, XCircle, BadgeCheck, CreditCard, Copy
} from 'lucide-react'
...
const STATUS_ORDER: Exclude<StatusKey, 'all'>[] = ['pending','processing','paid','shipped','completed','cancelled']
...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      setOrders(arr)
      setShippingInputs(Object.fromEntries(arr.map(o => [o._id, o.shippingNumber || ''])))
    } catch {
      setOrders([])
    } finally {
      setLoading(false)
  ...
  const orderTotal = (o: Order) =>
    o.amounts?.total ??
    o.items?.reduce((s, it) => s + (it.price || 0), 0) +
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">จัดการสถานะ อัปเดตเลขขนส่ง ค้นหา และลบออเดอร์</p>
        </div>
        <button
          onClick={fetchOrders}
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {filtered.map((o) => {
            ...
            return (
              <div key={o._id} className="rounded-2xl border border-orange-200 bg-white p-5 shadow-sm hover:shadow-md transition">
                {/* Header */}
                <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <button
                      ...
                      className="ml-2 inline-flex items-center gap-1 rounded-full bg-red-600 px-3 py-1 text-white text-xs font-semibold hover:bg-red-700"
                      title="ลบคำสั่งซื้อ"
                    >
                      ลบ
                    </button>
                    ...
                    <ul className="divide-y text-sm">
                      {o.items?.map((it, i) => (
                        <li key={i} className="flex items-center justify-between py-1.5">
                          ...
                          <span className="font-semibold text-slate-800">{it.price?.toLocaleString()} บาท</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="rounded-xl border border-orange-100 bg-orange-50/30 p-3">
                    <div className="mb-2 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-semibold">
                          {fmt(o.amounts?.subtotal ?? o.items?.reduce((s, it)=>s+(it.price||0)*(it.qty||1),0))}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                    ...
                    <div className="mt-3 grid gap-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        {o.shippingNumber && (
                          <button
                            onClick={() => navigator.clipboard?.writeText(o.shippingNumber || '')}
                            className="h-10 rounded-lg bg-white px-3 border border-orange-200 text-slate-700 hover:bg-orange-50"
                            title="คัดลอกเลขขนส่ง"
                          >
