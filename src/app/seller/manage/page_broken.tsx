...
import { PlusCircle, LogOut, Edit, Store, ShoppingCart, Package, Receipt, Printer, Users, DollarSign } from 'lucide-react'
...
export default function SellerManagePage() {
  ...
  useEffect(() => {
    ...
    if (needsCreate) {
      ...
      document.title = `${shopName} - จัดการสินค้า | TH-THAI SHOP`
    }
  }, [seller?.shopName, username, needsCreate])
      ...
      const list = data
        .map((o: any) => {
          ...
          const values = valuesRaw.map((v: any) => {
            if (typeof v === 'string') {
              return { value: v, price: 0, priceType: 'add' as const }
            } else if (typeof v === 'object' && v !== null) {
              return {
                value: ensureString(v.value || ''),
                price: Number(v.price) || 0,
                priceType: v.priceType === 'replace' ? 'replace' as const : 'add' as const
              }
            }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {needsCreate ? (
          ...
          <div>
            {/* Header Section */}
            <div className="bg-white/90 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl border border-white/20 p-6 sm:p-8 mb-6 sm:mb-8">
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-orange-500 via-pink-500 to-purple-600 flex items-center justify-center shadow-lg">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 mt-1">เพิ่ม แก้ไข และลบสินค้าในร้าน</p>
                    <div className="flex items-center gap-4 mt-2">
                      <span className="text-slate-500 text-sm bg-slate-100 px-3 py-1 rounded-full">
                        {products.length} สินค้า
                      </span>
                      <span className="text-slate-500 text-sm">ร้าน: {seller?.shopName || username}</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-gradient-to-r from-green-500 to-green-600 p-6">
                  ...
                  <div className="p-6">
                    <form onSubmit={handleProductUpload} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="md:col-span-2">
                        {/* Product Options Manager (same as admin) */}
                        <div className="mb-4">
                          ...
                          <ProductOptionsManager 
                            options={pOptions} 
                            basePrice={Number(pPrice) || 0}
                            onChange={setPOptions} 
                          />
                        </div>
                            ...
                            <div className="space-y-3">
                              {pOptions.map((o, i) => (
                                <div key={i} className="p-3 border rounded-lg bg-orange-50">
                                  <div className="font-semibold text-orange-700 mb-2">{o.name}</div>
                                  <div className="flex flex-wrap gap-2">
                                    {o.values.map((v, j) => (
                                      <span key={j} className="inline-flex items-center gap-1 px-3 py-1 bg-white border border-orange-200 rounded-full text-sm">
                                        <span>{v.value}</span>
                                        {v.price !== 0 && (
